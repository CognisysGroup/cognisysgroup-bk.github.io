...
Hi everyone, I wish you all are doing amazing. My name is Rajveersinh Parmar and I work here [@Cognisys](https://cognisys.co.uk/) as an Application Security Consultant. The aim of this blog is to make you aware of using Frida and to write your first Frida script for an Android application. 
...
Frida supports all the major operating systems including Windows, Linux and MacOS. The only prerequisite you need is to have python3 installed on your system. You can install Frida by running `pip install frida-tools`, to verify the installation, you can use `frida --version` or `frida-ps` command. If you can get the successful output of the above command, Frida is all set on your host machine. Now it's time to install it on the Android device.
...
Look for `frida-server-{version}-android-{architecture}` in the [releases](https://github.com/frida/frida/releases) section of the Frida GitHub repository and download the corresponding Frida Server with the latest version. Unzip the archive, further, you need to push the Frida Server to the device. To do so, use the command `adb push <Firda-Server> /data/local/tmp` 

![image](https://github.com/CognisysGroup/cognisysgroup.github.io/assets/25560539/57502600-642e-40bd-8519-177bb75479e1)

Further you need to modify the Frida-Server permissions and run it. To do so, follow the below commands:
...
To verify Frida-server is running correctly on Android, open a new terminal and enter `frida-ps -Uai` command. It will list the installed applications along with package names on the Android device.
...
`Androidmanifest.xml` contains the blueprint of the Android application. You can always start analysing the application source code from here. It can be found under the Resources directory. Here we want to find the code responsible for those restrictions and to do so, we need to start analysing the code from `MainActivity` as it's the first activity the application render. You can identify the `MainAcitvity` by checking the `MAIN` and `LAUNCHER` intent filter within it. 

![image](https://github.com/CognisysGroup/cognisysgroup.github.io/assets/25560539/15840906-d36c-482e-bf5e-16f0b702449f)

Since we got the path of the `MainActivity` which is `group.cognisys.fridame.MainActivity`, we can extract the package and open the `MainActivity`.
...
Then we have defined a constant variable named `MainAcitivity` assigning `Java.use()` to it. `Java.use()` is also provided by Frida's JavaScript API, it takes a parameter as a reference to the `MainActivity` class within the target application. It can allow you to access and modify the properties and methods of the `MainActivity` class.

In the next line, we have changed the implementation of the `isAdmin()` with our own function which will overwrite the actual code in runtime. `Console.log()` is just there to print a message on our terminal confirming we successfully have hooked the `isAdmin()`. At the last, we have made our function return `true` in order to satisfy the initial `if` condition so it can lead us to the `AdminArea` activity.

Let's run and test our script against the application. Make sure the Frida server on Android is up and running. Use the command `frida -U -l yourscript.js -f group.cognisys.fridame` to attach the script with the vulnerable application. The `-l` parameter is used to provide the Frida script and the `-f` flag accepts the package name of the application.

![image](https://github.com/CognisysGroup/cognisysgroup.github.io/assets/25560539/220953f6-9df4-4a0d-a2f0-1f05c9bb4a4e)

This will start the application with our Frida script attached to it. If you click the `ADMIN AREA` button, you would be able to bypass the restriction and access `AdminArea` activity as can be seen below.
...
The above code is responsible for root detection check. Let's understand the flow. There is an `onclick()` listener for the `userLogin` button which calls `Utils.isDeviceRooted()` method and stores its value in the `status` variable. Then there is an `if` condition which compares the values of the `status` variable with `"Device is safe."`. If that’s true then it will take us to another activity or else it will display the error `"Device is rooted, can't proceed further!"`.

Let's look at the implementation of `Utils.isDeviceRooted()` method. Double-click on a method to open the definition of it. Jadx-gui will open its class in a new tab.
...
Here, we have just passed the reference of the `Utils` class to `Java.use()` function. Then we changed its implementation to return the expected string and console log the message. So our final script would look like below.
