...
- Threads Stacks Encryption while sleeping.
- Reverting stomped module back to its original content while sleeping.

**The code** for this article is part of a basic ***Stager Shellcode Tasks Loader*** can be found on this **[Github](https://github.com/CognisysGroup/SweetDreams)** Repository.
...
Here's an example of data being put on the stack, while got passed as an argument to printf :
...
So as our program is sleeping the whole time , we don't want analyst or memory scanners to access this data, so we have to make sure that this data is encrypted by encrypting the whole Threads Stacks .
...
Let's see the following code that allocates some heap memory, then copy a string into that heap memory. we used `HeapCreate` to create a heap, and we allocated a block of memory within that heap using the `HeapAlloc`, then we copied the "data" string into this allocated memory using the `CopyMemory` :
...
So as our program is sleeping the whole time , we don't want analyst or memory scanners to access this data, so we have to make sure that this data is encrypted by encrypting the whole Heaps Allocations.


# Advanced Module Stomping

Advanced module stomping is a stealthy technique used by malware to inject its shellcode into a legitimate `.text` module's section to benefit from it's executable protection without the need of allocating an executable page.
...
In our project, we defined a structure that will hold informations about the shellcodes tasks that we will retrieve for our stager loader :
...
Before going further and encrypting the heap allocations we have to make sure that other threads won't access the heaps allocations while encrypting them , such behavior will crash the process, so we have to make sure to suspend all the threads and keep only the main thread 
...
After suspending the whole threads and keeping just the main thread active, we precede to encrypt the heaps allocations using `HeapEncryptDecrypt` function. it performs an in-place encryption and decryption operation on the allocated memory blocks within all process heaps, except the default process heap.
...
2. The function then retrieves the handles of all process heaps using the `GetProcessHeaps` function again and stores them in the "heaps" array.
    
3. Next, the function iterates through each heap in the "heaps" array. It skips the default process heap to avoid performing the encryption/decryption on it.
...
To make sure that actually those functions do the actual job, we will create a heap, allocate a memory within it, and copy the content of the "shellcode" array into the allocated memory, and print the heap allocation memory :
...
Let's monitor this Thread Stack encryption behavior.
